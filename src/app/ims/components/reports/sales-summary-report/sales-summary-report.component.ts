...
import { ReportsService, SalesSummary } from 'src/app/services/reports.service';
...
export class SalesSummaryReportComponent implements OnInit, OnDestroy {
  ...
  private async loadData(): Promise<void> {
    ...
    try {
      ...
      const obs = await this.reportsService.getSalesSummary(this.selectedPeriod, shopId);
      this.summary = await firstValueFrom(obs);
    } catch (e) {
      console.error(e);
      this.error = this.translate.instant('reports_sales_summary_error');
      this.summary = null;
    } finally {
      this.loading = false;
      this.cdr.markForCheck();
