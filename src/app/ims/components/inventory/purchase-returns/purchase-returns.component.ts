...
@Component({
  ...
  styleUrls: ['./purchase-returns.component.css', '../inventory.component.css'],
...
export class PurchaseReturnsComponent implements OnInit, OnChanges, AfterViewInit {
  ...
  onFilterChange() {
    ...
    if (this.dt) {
      ...
      if (this.selectedSupplier) {
        // Filter by purchase.supplier.name
        filters['purchase.supplier.name'] = { value: this.selectedSupplier.name, matchMode: 'equals' };
      }
  ...
  async onGetAllReturns() {
    try {
      const response = await this.purchaseReturnService.getReturns().toPromise();
      this.returns = response as PurchaseReturn[];
  ...
  openReturnDetailsDialog(returnData: PurchaseReturn | any): void {
    ...
    this.router.navigate(['/inventory/purchase-returns', returnId.toString()]);
  }
  ...
  exportPdf() {
    const modifiedReturns = this.returns.map(purchaseReturn => {
      const modifiedReturn = { ...purchaseReturn };
      if (purchaseReturn.purchase)
        modifiedReturn['Purchase'] = purchaseReturn.purchase.purchaseId;
